...
class OrderModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.order = Order.objects.create(
            type='T', customer=cls.user, store=cls.store)

        cls.order_item = OrderItem.objects.create(
            order=cls.order,
            product=cls.product,
            quantity=2,
            price=cls.product.price * 2,
        )
    ...
    def test_order_creation(self):
        ...
        self.assertTrue(self.order.order_number.startswith(
            now().strftime("%y%m%d")))
        self.assertTrue(self.order.order_number.endswith(
            f'{str(self.order.pk).zfill(3)}'))
    ...
    def test_order_item_creation(self):
        ...
        self.assertEqual(self.order_item.price,
                         self.product.price * self.order_item.quantity)
